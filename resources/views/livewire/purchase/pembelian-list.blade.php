<div class="bg-white p-4 rounded-xl shadow">
  <div class="mb-4 flex flex-wrap items-end justify-between gap-3">
    <div class="flex flex-wrap items-end gap-3">
      <div>
        <label class="block text-xs text-gray-500 mb-1">Cari (Nomor / Pemasok)</label>
        <input type="text" wire:model.live.debounce.400ms="cari"
               class="w-56 rounded-lg border px-3 py-2" placeholder="cth: PO-2025... / Sumber Makmur">
      </div>
      <div>
        <label class="block text-xs text-gray-500 mb-1">Pemasok</label>
        <select wire:model.live="pemasok_id" class="w-56 rounded-lg border px-3 py-2">
    ...
    <div class="flex items-center gap-2">
      <a href="{{ route('pembelian.index') }}" class="rounded border px-3 py-2 text-sm">+ Transaksi Baru</a>
      ...
      <tbody>
        @forelse($rows as $r)
          <tr class="border-b">
            ...
            <td class="p-2">{{ $r->pemasok->nama ?? $r->pemasok ?? '-' }}</td>
            ...
            <td class="p-2">
              <a href="{{ route('pembelian.show', $r->id) }}"
  ...
  <div class="mt-3 flex flex-wrap items-center justify-between gap-3">
    <div class="text-sm text-gray-600">
      Ditampilkan: <b>{{ $rows->firstItem() }}</b>–<b>{{ $rows->lastItem() }}</b> dari <b>{{ $rows->total() }}</b> baris.
