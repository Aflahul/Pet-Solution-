<div class="bg-white p-4 rounded-xl shadow">
    @if (session('ok'))
        <div class="mb-3 rounded bg-green-50 px-3 py-2 text-green-700 text-sm">
            {{ session('ok') }}
        </div>
    @endif
    ...
    <div class="mb-4 flex items-center justify-between gap-2">
        <div class="flex items-center gap-2">
            <input type="text" placeholder="Cari nama/desk…" wire:model.live.debounce.400ms="cari"
            ...
            <thead>
                <tr class="bg-gray-100">
                    <th class="p-2 cursor-pointer" wire:click="sortBy('nama')">
                        Nama @if ($sortField === 'nama')
                            <span>({{ strtoupper($sortDir) }})</span>
                        @endif
                    </th>
    ...
    @if ($showForm)
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center">
            <div class="w-full max-w-lg rounded-xl bg-white p-5 shadow-xl">
                <h2 class="mb-3 text-lg font-semibold">{{ $editingId ? 'Edit' : 'Tambah' }} Satuan</h2>
                ...
                <div class="mb-4">
                    ...
                    <textarea wire:model.live="deskripsi" class="w-full rounded border px-3 py-2"></textarea>
                    @error('deskripsi')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
    ...
    @if ($deleteId)
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center">
            <div class="w-full max-w-md rounded-xl bg-white p-5 shadow-xl">
                <p class="mb-4">Hapus data ini? Tindakan tidak bisa dibatalkan.</p>
